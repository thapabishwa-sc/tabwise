 ...
 *
 * Same lesson as reading pages: an await with no clock on it can stop a whole
 * pass. Inference is slow enough that this has to be generous, but a call that
 * has not answered in half a minute is not going to, and its batch is better
 * off falling back to titles than never finishing.
 */
const PROMPT_TIMEOUT_MS = 30000;

/** Resolve with `fallback` if `promise` has not settled in time. */
...
async function createSession(onProgress) {
  ...
  // Ask what this build allows before asking for it. A low temperature is what
  // keeps labels stable between passes, and requesting a value outside the
  // supported range makes create() throw — which used to drop us onto the
  // minimal path and silently lose the stability we were asking for. The spec
  // also requires temperature and topK to be set together, so they are clamped
  ...
  if (typeof LM.params === 'function') {
    try {
      ...
      if (p) {
        const maxTemp = typeof p.maxTemperature === 'number' ? p.maxTemperature : 1;
        const maxTopK = typeof p.maxTopK === 'number' ? p.maxTopK : 3;
        sampling = {
          temperature: Math.max(0, Math.min(maxTemp, 0.2)),
          topK: Math.max(1, Math.min(maxTopK, 3)),
        };
      }
    } catch { /* not exposed on this build; fall back to the plain request */ }
  }
  if (!sampling) sampling = { temperature: 0.2, topK: 3 };
...
async function promptOnce(userPrompt) {
  ...
  if (typeof base.clone === 'function') {
    ...
    if (copy) {
      try {
        return await withTimeout(promptForLabels(copy, userPrompt), PROMPT_TIMEOUT_MS, null);
      } finally {
  ...
  return withTimeout(promptForLabels(base, userPrompt), PROMPT_TIMEOUT_MS, null);
}
