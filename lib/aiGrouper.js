...
// Extra instruction when project/task grouping is requested.
const PROJECT_HINT =
  'Group tabs by the PROJECT or TASK they support (the work-stream they belong ' +
  'to), not just their generic topic. Tabs used together for one task should ' +
  'share a label, even across different sites.\n';
...
export async function groupTabs(tabs, opts = {}) {
  ...
  for (let start = 0; start < tabs.length; start += BATCH_SIZE) {
    ...
    const userPrompt =
      `Group these ${chunk.length} tabs.${existing}${modeLine}` +
      `Tabs:\n${lines.join('\n')}\n\n` +
