...
export function cleanTitle(title) {
  ...
  // Split on site-chrome separators and keep the longest remaining segment,
  // which is nearly always the content rather than the product name.
  const parts = s.split(SEPARATORS).map((p) => p.trim()).filter(Boolean);
  if (parts.length > 1) {
    const scored = parts.map((p) => ({
      p,
      // Prefer segments with real words over ones that are all site name.
      words: p.split(/\s+/).filter((w) => !STOPWORDS.has(w.toLowerCase())).length,
    }));
    scored.sort((a, b) => b.words - a.words || b.p.length - a.p.length);
    s = scored[0].p;
  }
