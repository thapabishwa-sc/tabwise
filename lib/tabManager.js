...
import {
  groupClusters, groupByInstruction, categorizeTab, proposeMerges,
...
import { gatherContent, readTabContent, hasContentAccess } from './pageContent.js';
...
/**
 * Wait for tabs that are still loading to finish.
 *
 * Grouping a tab mid-load reads a placeholder title and, with page reading on,
 * an unbuilt document — so a full pass would judge some tabs on nothing and
 * then have to be corrected. Waiting costs a moment once; a wrong group costs a
 * correction and, worse, teaches the wrong thing.
 *
 * Bounded, because a tab can stay 'loading' forever on a slow or broken page,
 * and one such tab must not hold up everything else.
 */
async function waitForTabsToLoad(tabIds, { timeoutMs = 8000, pollMs = 250 } = {}) {
  ...
  while (waiting.size > 0 && Date.now() < deadline) {
    for (const id of [...waiting]) {
      try {
        ...
        // A discarded tab will never load on its own, and reloading it to read
        // it would be a rude thing to do to someone's browser.
        if (t.status === 'complete' || t.discarded) waiting.delete(id);
      } catch {
        waiting.delete(id); // closed
      }
    }
    if (waiting.size === 0) break;
    await new Promise((r) => setTimeout(r, pollMs));
  }
  return { settled: tabIds.length - waiting.size, stillLoading: waiting.size };
}

/** Tell the popup how a long pass is progressing, if it is listening. */
function reportProgress(phase, done, total) {
  chrome.runtime.sendMessage({ type: 'ORGANIZE_PROGRESS', phase, done, total })
    .catch(() => { /* no popup open */ });
}
...
/** Drop the stored explanations; they describe a grouping that no longer holds. */
export async function clearReasons() {
  try {
    await chrome.storage.session.remove(REASON_KEY);
  } catch { /* session storage unavailable */ }
}
...
export async function organizeAllTabs() {
  ...
  try {
    ...
    // --- Phase 0: let every tab finish loading -----------------------------
    // Nothing is grouped until everything that can say what it is, has. A tab
    // judged mid-load is judged on a placeholder.
    const loading = allTabs.filter((t) => isEligible(t) && t.status === 'loading');
    if (loading.length > 0) {
      reportProgress('loading', 0, loading.length);
      await waitForTabsToLoad(loading.map((t) => t.id));
      allTabs = await chrome.tabs.query({}); // titles will have changed
    }
    ...
    // --- Phase 1: read every page, before grouping any of them -------------
    // Gathered across all windows first so that no window is grouped on less
    // information than another, and so the reading happens once.
    const eligible = [...byWindow.values()].flat();
    let content = new Map();
    if (settings.readPageContent !== false && await hasContentAccess()) {
      content = await gatherContent(eligible, {
        onProgress: (done, total) => reportProgress('reading', done, total),
      });
    }
    const withContent = (tab) => {
      const text = content.get(tab.id);
      return text ? { ...payload(tab), content: text } : payload(tab);
    };
    ...
    const nameClusters = ai
      ...
      : null;
    // Only worth asking when the model is there and the user wants it.
    const mergePass = (ai && settings.aiMergePass !== false)
      ? (groups) => proposeMerges(groups)
      : null;

    // --- Phase 2: decide, then apply ---------------------------------------
    let organized = 0;
    let windowsDone = 0;
    for (const [windowId, tabs] of byWindow) {
      if (tabs.length === 0) continue;
      reportProgress('grouping', windowsDone, byWindow.size);

      const { assignments, byLabel } = await resolveGroups(tabs.map(withContent), {
        settings,
        memory: learned,
        nameClusters,
        proposeMerges: mergePass,
      });

      // Nothing here is learned. Memory records what YOU decide — a rename, a
      // drag, a captured task — and not what a pass guessed.
      //
      // Automatic groupings used to be learned too, on the reasoning that a
      // recurring task would then be recognized without an AI call. The cost
      // turned out to be far higher than the benefit: one wrong grouping became
      // a permanent profile, and a profile is a magnet — it pulls in more tabs,
      // which are grouped under it, which teaches it further. A profile was
      // observed holding two unrelated Confluence pages and both "onboarding"
      // and "upgrade", so every page of either kind recalled the same task, and
      // no amount of fixing the scoring dislodged it.
      //
      // The benchmark could not have caught this: its warm pass learns from the
      // GOLD grouping, so it only ever measured learning from correct answers.
    ...
    reportProgress('done', byWindow.size, byWindow.size);
    return {
      organized,
      mode: ai ? 'ai' : 'fallback',
      readPages: content.size,
    };
  } finally {
    busy = false;
  }
...
/**
 * Group only the tabs that are currently loose, leaving existing groups alone.
 *
 * "Group all tabs" reorganizes everything, which is the wrong tool once you are
 * happy with most of your groups — and loose tabs are deliberately common now
 * that a tab stays uncategorized until it has loaded and nothing is placed on
 * weak evidence. This tidies the stragglers without disturbing what is settled.
 *
 * The whole window is still scored, because relatedness is judged by how rare a
 * signal is across the tabs actually open; only the loose ones are moved.
 */
export async function organizeLooseTabs(windowId) {
  const ai = await aiAvailable();
  ...
  try {
    ...
    const tabs = (await chrome.tabs.query(winId != null ? { windowId: winId } : {}))
      .filter(isEligible);
    const loose = new Set(
      tabs.filter((t) => (!t.groupId || t.groupId === -1) && !isManuallyGrouped(t, managed))
        .map((t) => t.id),
    );
    if (loose.size === 0) return { organized: 0 };

    let content = new Map();
    if (settings.readPageContent !== false && await hasContentAccess()) {
      content = await gatherContent(tabs);
    }
    const withContent = (tab) => {
      const text = content.get(tab.id);
      return text ? { ...payload(tab), content: text } : payload(tab);
    ...
    const { assignments } = await resolveGroups(tabs.map(withContent), {
      settings,
      memory,
      nameClusters: ai ? (clusters, o) => groupClusters(clusters, o) : null,
      proposeMerges: (ai && settings.aiMergePass !== false)
        ? (groups) => proposeMerges(groups)
        : null,
    });

    // Apply to the loose tabs only; everything already grouped stays put.
    const byLabel = new Map();
    for (const [tabId, a] of assignments) {
      if (!loose.has(tabId)) continue;
      if (!byLabel.has(a.label)) byLabel.set(a.label, []);
      byLabel.get(a.label).push(tabId);
    }
    ...
    await saveReasons(new Map([...assignments].filter(([id]) => loose.has(id))));
    if (settings.collapseInactive) await collapseInactiveGroups(winId);
...
export async function autoGroupTab(tabId) {
  ...
  // Read the page once it has settled, so the stages below judge what the page
  // says rather than only what its title and address say.
  let self = payload(tab);
  if (settings.readPageContent !== false && await hasContentAccess()) {
    const text = await readTabContent(tab);
    if (text) self = { ...self, content: text };
  }
  ...
  const recalled = recallForTabs(memory, [self]);
  ...
  const near = relatedGroupFor(self, others, {
  ...
  if (!(await aiAvailable())) {
    // One tab, and nothing describes it: leave it loose rather than filing it
    // under its hostname. "Tidy loose" or a full pass can revisit it once there
    // are other tabs to relate it to.
    const fb = nameCluster({ tabs: [self], key: identitiesOf(self)[0] || null });
  ...
  const label = await categorizeTab(self, existingLabels, { projectMode: settings.aiProjectMode });
  if (!label) return;
  ...
  if (minSize > 1 && !existingLabels.includes(label) && !namesWork(self)) return;
...
// --- Capture the current tabs as a benchmark fixture ---

/** Query parameters that can carry credentials and must not leave the browser. */
const SENSITIVE_PARAM = /(token|key|secret|password|passwd|pwd|auth|session|sid|sig|signature|credential|access|bearer|api[-_]?key)/i;

/**
 * Strip anything credential-shaped from a URL, keeping what grouping uses.
 *
 * A fixture is meant to be pasted into a bug report, so it leaves the browser.
 * Grouping reads the host and path and almost never the query, so dropping
 * suspicious parameters costs the fixture nothing and removes the main way a
 * captured URL could carry a secret.
 */
...
/**
 * Snapshot the current window as a benchmark fixture (see scripts/fixtures/).
 *
 * Every grouping bug so far has been found by real tabs and fixed by hand-
 * transcribing them into a fixture, which is the slow and error-prone step.
 * This produces one directly, with `gold` pre-filled from where each tab
 * currently sits — so reporting a bug means correcting the few that are wrong
 * rather than typing all of them out.
 */
export async function captureFixture(windowId) {
  let winId = windowId;
  if (winId == null) {
    try { winId = (await chrome.windows.getCurrent()).id; } catch { /* noop */ }
  }
  ...
  try {
    const groups = await chrome.tabGroups.query(winId != null ? { windowId: winId } : {});
    titleById = new Map(groups.map((g) => [g.id, g.title || '']));
  } catch { /* tabGroups unavailable */ }

  const reasons = await getReasons();

  // Fixture ids are 1..n, so an opener has to be translated into that space —
  // and dropped when the tab it points at was not captured. The benchmark reads
  // this field as `opener`; anything else silently loses link trails, which are
  // the strongest signal a fixture can carry.
  const indexById = new Map(tabs.map((t, i) => [t.id, i + 1]));

  return {
    name: 'captured',
    _howto: 'Correct the "gold" values to the grouping you WANTED, drop any tabs '
      + 'that are noise, rename, and save under scripts/fixtures/. Then run '
      + 'node scripts/bench.mjs. "gold" is pre-filled with where each tab landed, '
      + 'and "_why" records which stage put it there — delete _why before saving.',
    description: 'Captured from a real window. Describe what should have happened.',
    settings: { minGroupSize: 1 },
    tabs: tabs.map((t, i) => {
      const r = reasons[t.id];
      return {
        id: i + 1,
        gold: (t.groupId && t.groupId !== -1 ? titleById.get(t.groupId) : '') || 'Ungrouped',
        title: t.title || '',
        url: redactUrl(t.url),
        ...(indexById.has(t.openerTabId) ? { opener: indexById.get(t.openerTabId) } : {}),
        ...(r ? { _why: `${r.reason}/${r.via}` } : {}),
      };
    }),
  };
}
