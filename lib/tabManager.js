    ...
    for (const [windowId, tabs] of byWindow) {
      ...
      if (forTasks.length > 0) {
        ...
        if (ai) {
          ...
          //
          // A cluster with no key was built from a link trail, which is a task
          // signal in its own right and does not need the model. So it is still
          // labeled as ONE group, named after the tab the trail started from,
          // rather than being split back apart by hostname.
          for (const c of clusters) {
            const label = labelForKey(c.key)
              || c.tabs.map((t) => fallbackGroupLabel(t.url)).find(Boolean);
            if (!label) continue; // nothing nameable (e.g. an odd URL)
            exempt.add(label);
            for (const t of c.tabs) addTo(label, t.id);
          }
        }
      }
