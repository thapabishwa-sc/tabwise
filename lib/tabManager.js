...
export async function organizeAllTabs() {
  ...
  try {
    ...
    for (const [windowId, tabs] of byWindow) {
      ...
      // A group named after a site, or the leftover bucket, is an admission
      // that nothing was known — learning it would make that guess permanent
      // and keep reproducing it long after the reason was fixed.
      const guessed = new Set();
      for (const [, a] of assignments) {
        if (a.reason === 'host' || a.reason === 'misc') guessed.add(a.label);
      }
  ...
  try {
    ...
    for (const [label, ids] of byLabel) {
      try {
        ...
        learned = observe(learned, label, ids.map((id) => payload(tabsById.get(id))).filter(Boolean));
      } catch (e) {
        console.warn(`AI Tab Grouper: failed to group "${label}":`, e);
      }
    }
