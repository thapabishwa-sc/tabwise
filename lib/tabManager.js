 ...
 * The rule's `match` is tested as a substring of "hostname + path", so rules
 * can be path-specific (e.g. "atlassian.net/wiki" → Confluence). Rules are
 * evaluated in order; the first match wins, so list specific rules first.
 */
function pinnedLabelFor(url, pinnedRules) {
  ...
  try {
    const u = new URL(url);
    target = (u.hostname + u.pathname).toLowerCase();
  } catch { return null; }
  for (const rule of pinnedRules) {
    if (rule && rule.match && target.includes(rule.match.toLowerCase())) {
...
export async function organizeAllTabs() {
  ...
  try {
    ...
    for (const [windowId, tabs] of byWindow) {
      ...
      if (forAi.length > 0) {
        if (ai) {
          const labelMap = await groupTabs(forAi.map(payload)); // tabId → label
...
export async function autoGroupTab(tabId) {
  ...
  const label = await categorizeTab(payload(tab), existingLabels);
  if (!label) return;
