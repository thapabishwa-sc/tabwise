...
import {
  resolveGroups, pinnedLabelFor, hostRuleOpts, REASONS,
} from './resolve.js';
import {
...
export async function autoGroupTab(tabId) {
  ...
  // 6. Nothing recognized it. Ask the model, or name it from its own title.
  if (!(await aiAvailable())) {
    const fb = nameCluster({ tabs: [payload(tab)], key: taskKey(tab.url) })
