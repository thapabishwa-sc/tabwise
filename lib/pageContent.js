...
/**
 * Injection into a live page can hang forever, not fail.
 *
 * chrome.scripting.executeScript returns a promise that neither resolves nor
 * rejects when the target renderer cannot run script — a tab showing a modal
 * dialog, a page stuck in a long-running loop, a frozen background tab. The
 * await simply never returns, its worker never finishes, and Promise.all over
 * the pool never settles: progress stops mid-count and the busy flag stays set,
 * which silently disables grouping for every other tab until the service worker
 * restarts.
 *
 * So every read is raced against a clock. A live page answers in milliseconds;
 * anything slower is not worth waiting for and is treated as unreadable.
 */
const READ_TIMEOUT_MS = 1500;

/** Ceiling for a whole gather, so no window can stall a pass. */
const GATHER_BUDGET_MS = 20000;

/**
 * Resolve with `fallback` if `promise` has not settled within `ms`.
 *
 * The underlying promise is abandoned rather than cancelled — there is no way
 * to cancel an injection — so it may stay pending. That is harmless: nothing
 * awaits it any more.
 */
...
function extractInPage() {
  ...
  // Chrome, not content. Every page of an app repeats its navigation, sidebar
  // and footer, so reading them makes two unrelated pages of the same app look
  // like the same work: two Confluence pages scored 5.53 on "attachments",
  // "templates", "restrictions" and "calendars" and were grouped together,
  // which is grouping by domain by another route.
  const BOILERPLATE = [
    'nav', 'header', 'footer', 'aside', 'script', 'style', 'noscript', 'svg',
    'button', 'select', 'option', 'form', 'label',
    '[role="navigation"]', '[role="banner"]', '[role="contentinfo"]',
    '[role="complementary"]', '[role="search"]', '[role="toolbar"]',
  ...
  let body = '';
  try {
    // Cloned so the live page is never modified, then stripped of chrome.
    const copy = container.cloneNode(true);
    copy.querySelectorAll(BOILERPLATE).forEach((el) => el.remove());
    body = clean(copy.textContent).slice(0, 600);
  } catch {
    body = clean(container.textContent || '').slice(0, 600);
  }

  // Headings from the stripped region only, for the same reason: a sidebar's
  // section titles are h2s too.
  let headings = [];
  try {
    const copy = container.cloneNode(true);
    copy.querySelectorAll(BOILERPLATE).forEach((el) => el.remove());
    headings = [...copy.querySelectorAll('h1, h2')]
...
function isReadable(tab) {
  ...
  // Neither a discarded nor a frozen tab has a renderer that can run script.
  // Chrome freezes background tabs to save memory, and an injection into one
  // is a a request that may never be answered.
 ...
 */
/** Exposed for testing the race, which is the part that went wrong. */
export const __testing = { withTimeout, READ_TIMEOUT_MS, GATHER_BUDGET_MS };
 ...
 */
export async function readTabContent(tab, timeoutMs = READ_TIMEOUT_MS) {
  ...
  // Raced, because the promise above can stay pending forever.
...
export async function gatherContent(tabs, opts = {}) {
  ...
  const queue = [...pending];
  const deadline = Date.now() + (opts.budgetMs ?? GATHER_BUDGET_MS);
  const worker = async () => {
    for (;;) {
      const tab = queue.shift();
      if (!tab) return;
      // Past the budget, drain what is left as unread rather than stopping —
      // the count must still reach the total, or the UI reports a number that
      // never completes.
      const text = Date.now() < deadline
        ? await readTabContent(tab, opts.timeoutMs)
        : '';
      out.set(tab.id, text);
      if (text) cache[tab.id] = { url: tab.url, text };
      done++;
      if (opts.onProgress) opts.onProgress(done, total);
    }
  };
