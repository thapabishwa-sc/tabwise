 ...
 *
 * Keys come in two tiers, because a URL can name either a piece of work or a
 * place where lots of unrelated work lives:
 *
 *   STRONG — one specific work item: a ticket (`ticket:WEB-101`), a pull request,
 *     a single document. Two tabs sharing a strong key are the same task, full
 *     stop.
 *   WEAK — a container holding many work items: a repository, a Jira project, a
 *     wiki space, a Slack channel. Three unrelated tickets can all touch one
 *     monorepo, so a shared repository proves nothing on its own; treating it as
 *     proof merges every ticket in the repo into one useless group.
 *
 * lib/affinity.js unions freely on strong keys, and only cautiously on weak
 * ones — never joining two tabs that already belong to different work items.
 *
 * Two outputs per tab:
 *   - readTask(url) → { key, weak, display, host }
 ...
 * so known non-ticket prefixes are rejected outright.
 *
 * The prefix list does that work on its own, which is why no minimum digit
 * count is imposed: requiring two digits would reject real single-digit tickets
 * like SEARCH-9, and a pull request titled "SEARCH-9: add cache" is exactly the
 * case that most needs to find its ticket.
 *
 * @param {string} text
...
/** The full ticket reference ('WEB-101') when it is confidently a ticket. */
function confidentTicketRef(text) {
  const project = confidentTicketProject(text);
