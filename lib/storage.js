...
const DEFAULTS = {
  ...
  //   'manual' (default) — only when you click "Group all" / use a command /
  //              capture a task. Tabs never move on their own.
  //   'auto'   — also group each new/navigated tab as it settles.
  groupingMode: 'manual',

  // When the AI groups, bias toward the PROJECT/TASK a tab supports rather than
  // its generic topic (e.g. tabs for one work-stream share a label).
  aiProjectMode: false,
