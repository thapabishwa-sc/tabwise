...
import { identitiesOf } from './context.js';
...
/**
 * Every identifier a tab carries, from its URL and its title.
 *
 * These come from the context engine rather than a per-site registry, so a task
 * on an in-house tracker is remembered as precisely as one on Jira. They are
 * absolute — independent of which other tabs are open — which is what makes
 * them safe to write down and match against tomorrow.
 */
export function keysForTab(tab) {
  return identitiesOf(tab);
}
