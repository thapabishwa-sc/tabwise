...
// 7: v6 was not enough. A profile polluted by an automatic pass and THEN
//    renamed carried userNamed: true and survived, still holding both halves of
//    a wrong grouping. Feature provenance was never recorded, so a mixed
//    profile cannot be cleaned selectively — every profile is dropped and only
//    your names and your pins are kept.
...
function migrate(stored) {
  ...
  // Every profile goes. v6 tried to keep the user-named ones, and that missed
  // the case that prompted all of this: a profile polluted by an automatic pass
  // and then renamed is user-named AND wrong. Nothing recorded which features
  // came from a person and which from a guess, so there is no honest way to
  // separate them — and a profile that is half wrong is worse than none, since
  // it is a magnet.
  //
  // What survives is what was unambiguously yours:
  //   aliases — the name you chose, still replacing the proposed one
  //   pins    — the pages you filed by hand, still filed there
  //
  // Profiles rebuild from your corrections, which since v6 is the only way they
  // are built at all.
  return {
    ...emptyMemory(),
...
/**
 * Forget every profile while keeping your names and your pins.
 *
 * Between "forget one task" and "forget everything" there was nothing, and the
 * useful middle is exactly this: what it thinks it recognizes is wrong, but the
 * names you chose and the pages you filed are still right.
 */
export function resetProfiles(memory) {
  return { ...memory, tasks: {} };
}
