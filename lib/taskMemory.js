...
/**
 * Evidence needed to recognize a task when nothing identifies it outright.
 *
 ...
 * its scores are not divided down. Roughly "two distinctive words, or one plus
 * a matching path".
 */
const RECALL_THRESHOLD = 1.5;
...
export function recallForTabs(memory, tabs) {
  ...
  for (const [label, task] of Object.entries(memory.tasks)) {
    ...
    const confident = top.identity > 0 || (top.subject + top.container) >= RECALL_THRESHOLD;
...
function mergeInto(task, tabs, now) {
  ...
  for (const t of tabs || []) {
    for (const key of profileFeaturesOf(t)) features.add(key);
  }
  return {
    ...task,
