...
export function clusterTabs(tabs, opts = {}) {
  ...
  // A tab is unioned on every key it carries, so a tab holding two identities
  // bridges the two clusters.
  const keysOf = new Array(list.length);
  const firstWithKey = new Map();
  for (let i = 0; i < list.length; i++) {
    const urlKey = readTask(list[i].url).key;
    const titleKey = titleTaskKey(list[i].title);
    const keys = [...new Set([urlKey, titleKey].filter(Boolean))];
    keysOf[i] = keys;
