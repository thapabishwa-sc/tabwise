 ...
 *
 * Finally, clusters that are plainly ABOUT the same thing are merged on their
 * shared vocabulary. A ticket and the runbook page documenting it are different
 * work items in different containers with no link between them, so nothing
 * above catches them — only the word they have in common does.
 *
...
import { summarizeTitles, looksLikeIdentifier, titleTokens } from './summarize.js';
...
export function clusterTabs(tabs, opts = {}) {
  ...
  // --- 4. Shared vocabulary between whole clusters ---
  if (opts.useTopics !== false) mergeByTopic(list, dsu, via);
...
/** Shared vocabulary strong enough to call two clusters the same work. */
const TOPIC_MERGE_THRESHOLD = 0.5;

/**
 * How much a shared word says about two tabs being the same work.
 *
 * Two forces, and both are needed. A word's SPECIFICITY: "onboarding" pins down
 * a subject, "org" does not, and length is a decent proxy for that — a digit
 * makes a short word specific too, since "Q3" and "v2" name real things.
 * A word's SPREAD: a word shared by two clusters out of ten is a signal, and
 * the same word shared by eight of them is just this person's vocabulary, so it
 * is divided out.
 *
 * Without the specificity term this cannot work. In a window holding
 * "[single-org] customerpoc - onboarding" and "[multi-org] billing mismatch",
 * the words "onboarding" and "org" are each shared by exactly two clusters, so
 * frequency alone rates them identically — and one of them is the subject of
 * the work while the other is a deployment tag.
 */
function tokenWeight(token, clusterFreq) {
  const specificity = /\d/.test(token)
    ? 1
    : Math.min(1.5, Math.max(0.25, token.length / 8));
  return specificity / Math.max(1, clusterFreq);
}

/**
 * Merge clusters that share enough distinctive vocabulary.
 *
 * Runs last, on whole clusters rather than individual tabs, so the evidence is
 * everything a cluster is about rather than one title. Merges only — it can
 * never split what the earlier stages established.
 *
 * @param {Array<object>} list  tabs, in cluster-membership order
 * @param {{find:Function, union:Function}} dsu
 * @param {string[]} via  per-tab join reasons, updated in place
 */
function mergeByTopic(list, dsu, via) {
  // Vocabulary of each cluster as it currently stands.
  const tokensByRoot = new Map();
  for (let i = 0; i < list.length; i++) {
    const root = dsu.find(i);
    if (!tokensByRoot.has(root)) tokensByRoot.set(root, new Set());
    const set = tokensByRoot.get(root);
    for (const w of titleTokens(list[i].title)) set.add(w);
  }
  const roots = [...tokensByRoot.keys()];
  if (roots.length < 2) return;

  // How many clusters use each word.
  const clusterFreq = new Map();
  for (const set of tokensByRoot.values()) {
    for (const w of set) clusterFreq.set(w, (clusterFreq.get(w) || 0) + 1);
  }

  // Score every pair, then apply the qualifying merges. Scores are computed
  // against the original clustering so that one merge cannot cascade into a
  // chain of weaker ones.
  const merges = [];
  for (let a = 0; a < roots.length; a++) {
    for (let b = a + 1; b < roots.length; b++) {
      const A = tokensByRoot.get(roots[a]);
      const B = tokensByRoot.get(roots[b]);
      let score = 0;
      for (const w of A) {
        if (B.has(w)) score += tokenWeight(w, clusterFreq.get(w) || 1);
      }
      if (score >= TOPIC_MERGE_THRESHOLD) merges.push([roots[a], roots[b]]);
    }
  }
  ...
  // Mark the tabs whose placement this stage explains.
  for (let i = 0; i < list.length; i++) {
    if (via[i] === 'alone' && merged.has(dsu.find(i))) via[i] = 'topic';
  }
}
