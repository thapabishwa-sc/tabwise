...
export async function resolveGroups(tabs, opts = {}) {
  ...
  if (unnamed.length > 0) {
    ...
    for (const cluster of unnamed) {
      ...
      const described = nameCluster(cluster);
      // The site name is the last resort, and it is only allowed to name a
      // group that something else already assembled. A lone tab nobody can
      // describe stays loose instead: naming it after its host is domain
      // grouping, which is the thing this all exists to avoid, and an
      // uncategorized tab is honest where a wrong group has to be corrected.
      const bySite = cluster.tabs.length > 1
        ? cluster.tabs.map((t) => fallbackGroupLabel(t.url)).find(Boolean)
        : null;
      const label = fromModel || described || bySite;
      if (!label) continue; // nothing nameable — leave it out of every group

      const reason = fromModel ? 'ai' : (described ? 'summary' : 'host');
      cluster.label = label;
