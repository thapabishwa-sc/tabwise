...
function isOpaqueId(seg) {
  ...
  // An unbroken alphanumeric run.
  if (/^[A-Za-z0-9_]+$/.test(seg) && seg.length >= 8) {
    return /\d/.test(seg) && /[A-Za-z]/.test(seg);
  }

  // Generated keys often contain dashes — every Google Docs, Sheets and Drive
  // id does — and rejecting dashes outright meant none of them were recognized
  // as identifying anything. A document with no identity has nothing to stop
  // container evidence adopting it, and two tabs of the same document did not
  // recognize each other.
  //
  // Length plus mixed case plus a digit is what separates such a key from a
  // readable slug: "1ri2QCixIfitJYeHGn-ZMEXZeLmywYBPh8kRs8O75pzY" qualifies,
  // while "getting-started-2" has no capitals and
  // "NG-SaaS-onboarding-offboarding" has no digits.
  if (/^[A-Za-z0-9_-]+$/.test(seg) && seg.length >= 16) {
...
export function extractFeatures(tab) {
  ...
  if (u && /^https?:$/.test(u.protocol)) {
    ...
    // Namespaced by host. A generated id identifies a document only within the
    // site that generated it: a Confluence page id and some unrelated site's
    // numeric path segment can be the same digits, and un-namespaced they
    // produced the same identity — worth 3.0, decisive — merging two tabs that
    // had nothing in common but a coincidence of numbers.
    for (const seg of segs) {
      if (isOpaqueId(seg)) add(`opaque:${host}/${seg}`);
    }
 ...
 *
 * A task is its work and its vocabulary — NOT the place it happens to live. So
 * hosts and path prefixes are left out of a profile entirely.
 *
 * They used to be included, and they were the majority of what a profile held:
 * one Confluence page contributed five cumulative prefixes (`/wiki`,
 * `/wiki/spaces`, `/wiki/spaces/en`, and so on) against three real signals.
 * Worse, a profile is scored without any rarity discount, so an unrelated page
 * in the same wiki space matched five of them for a container score of 5.35 and
 * was recalled as that task. Memory was doing domain grouping, and it was the
 * most durable form of it — a learned mistake outlives the pass that made it.
 *
 * Interior path words are dropped for the same reason: a profile carrying
 * "monorepo" as subject matter would match every tab in the repository.
 */
export function profileFeaturesOf(tab) {
  const { features, interior } = extractFeatures(tab);
  const out = [];
  for (const key of features.keys()) {
    if (key.startsWith('path:') || key.startsWith('host:')) continue;
